...
本变更覆盖采集、解析、标准化、检索展示与任务可观测全链路，属于跨模块协同能力建设。当前系统是 Java 17 + Maven 多模块项目（`facade`、`infrastructure`、`service`、`bootstrap`），首期约束为 MyBatis（非 MyBatis-Plus）、不引入 Redis。当前交付形态调整为：后端统一提供 `/h5/*` 查询 API，前端以 `web/`（Vue3 + Vite）网页承载。

核心业务约束包括：
- 需要稳定执行“每日自动监测公众号新文章并采集”的定时任务；
- 需要支持手动输入文章 URL 的补采（以 crawler CLI 入口实现），并纳入同一任务体系；
...
- 为后续小程序通过 web-view 复用提供稳定 API 与数据契约（前端页面本期先以 web 页面落地）。
...
### 决策 4：查询采用数据库条件检索 + 索引优化，暂不引入缓存
- 选择：基于 MyBatis 动态 SQL 实现关键词搜索、多条件筛选与排序；当前排序范围为发布时间升降序（`desc/asc`），通过索引保障首期性能。
- 原因：符合“不引入 Redis”的约束，数据规模早期可通过 SQL 与索引满足。
- 备选方案：引入搜索引擎或 Redis 缓存。放弃原因是超出首期范围与运维复杂度。

### 决策 5：薪资按原文字符串存储，发布时间作为排序主键
- 选择：薪资字段首期采用字符串原样存储（如“120/2h”“面议”“自报价”），不做数值化建模；排序仅基于发布时间升降序。
...
- [查询能力与产品诉求错位] -> Mitigation：在 OpenSpec 中明确本期不含独立详情页与薪资数值排序，避免实现与验收标准不一致。

## Migration Plan

1. 先上线数据库变更（文章原文、任务、结构化信息、发布时间与检索字段），保持向后兼容。
2. 部署采集与解析任务处理链路，先以低频定时运行并观察成功率与失败分类。
3. 开启手动补采入口（crawler CLI）并与自动任务同库同状态机运行，验证重试链路可用。
4. 上线查询接口与 web 页面，默认展示全部解析成功数据。
...
- 手动补采后续是否需要升级为 HTTP 接口（当前为 crawler CLI）并补充鉴权？
- 是否需要在下一期新增详情接口（当前由列表直接返回来源链接跳转原文）？
- 是否需要引入薪资数值化解析与“薪资高/低优先”排序规则？
